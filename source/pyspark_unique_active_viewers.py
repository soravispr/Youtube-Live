...
sheet = client.open("youtube_live").worksheet('pyspark_unique_active_viewers') # change the cheet name here
...
def update_data(x):
    ...
    if data: # check if there is data
        for key, c in data: # loop each user
            sheet.append_row([key, dt_now, c]) # append row to gg spreadsheet

input_data = kvs.map(extract_data) # check if it's new line
filtered_data = input_data.filter(lambda x: x[0]) # filter out None
unique_viewers = filtered_data.reduceByKey(lambda a, b: 1).map(lambda x: (x[0].split(':::')[0], 1))
unique_viewers_count = unique_viewers.reduceByKey(lambda a, b: a + b)
unique_viewers_count.foreachRDD(update_data)
