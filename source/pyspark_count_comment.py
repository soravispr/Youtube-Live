...
sheet = client.open("youtube_live").worksheet('pyspark_count_comment') # change the cheet name here
...
def extract_data(x):
    if not x[1] or len(x[1].split(':::')) < 3:
        ...
        return (x[1].encode('utf8').decode('unicode_escape').split(':::')[0][1:], 1)
...
comment_count = filtered_data.reduceByKey(lambda a, b: a + b) # count comments
